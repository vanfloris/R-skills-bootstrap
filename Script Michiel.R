...
gamma.hat.1 <- ols.lm1$coefficients[2:5,] - xi.hat.0 %*% ols.lm2$coefficients[2:5,]
...
for (i in (p + 1):(t + 2*p)){ # Generate series with e ~ N(0,1)
  ...
  est.VAR[, i] <- (alpha.hat %*% beta.hat.r + A.2)%*% est.VAR[, i-1] + alpha.hat +
      rnorm(k, 0, 1)+ gamma.hat.1 %*%
